...
const createOrder = (request, response) => {
  ...
  const userid = request.user;
  pool.query('INSERT INTO orders (total,status,userid,created,modified) VALUES ($1, $2, $3,$4,$5) RETURNING *', [total,status,userid,utc().toISOString(),utc().toISOString()], (error, result) => {
    if (error) {
      throw error
    }
